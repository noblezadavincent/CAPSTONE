...
if(isset($_POST["register"])){
	...
	if ($usertype == "freelancer") {
		...
		if($result->num_rows > 0){
			...
			$sql = "INSERT INTO freelancer (username, password, name, email, contact_no, address, gender, birthdate, photo, facebook, gmail, twitter, linkedin) 
					VALUES ('$username', '$password', '$name','$email','$contactNo','$address','$gender','$birthdate', '$photo', '$fb', '$gm', '$tw', '$li')";
			$result = $conn->query($sql);
			if($result == true){
				$_SESSION["Username"]=$username;
				$_SESSION["Usertype"]=1;
				header("location: outsidesignup.php");
			}
			else{
		...
		if($result->num_rows > 0){
			...
			unset($_SESSION["errorMsg2"]);
			$sql = "INSERT INTO employer (username, password, Name, email, contact_no, address, gender, birthdate,photo, facebook, gmail, twitter, linkedin) 
					VALUES ('$username', '$password', '$name','$email','$contactNo','$address','$gender','$birthdate','$photo', '$fb', '$gm', '$tw', '$li')";
			$result = $conn->query($sql);
			if($result==true){
				$_SESSION["Username"]=$username;
